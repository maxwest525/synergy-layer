...
| **CODE-16** | **`fire.marky.systems` returns 401 from Caddy, not from Firecrawl, so no Firecrawl key can fix it.** Measured 2026-08-29: `/`, `/is-production` and `/v2/scrape` all return HTTP 401 with `server: Caddy` and a 12-byte `text/plain` body reading `Unauthorized`. There is **no `WWW-Authenticate` header**, so it is not offering Basic auth, and `Authorization: Bearer`, `-u user:pass` and `X-API-Key` all get the identical 401. Firecrawl's own auth errors are JSON; this is plain text from the reverse proxy. The request never reaches Firecrawl. | The stored probe agrees: `selfhosted_firecrawl` is `integration_state: real`, `health: failing`, `probe_outcome: http_error`, `statusCode: 401` against `https://fire.marky.systems/is-production`. **Replacing `SELFHOSTED_FIRECRAWL_API_KEY` will not change this** — the fix is in the Caddy site config on the operator's box. Not determinable from here: whether Caddy is doing an IP allowlist (this session and AOOS both egress from addresses the operator's box has never seen) or expecting a differently-named header. | Verified |

...
| **OP-9** | **Publish the website.** brittmove PR #4 (prerender) is merged to `main` and **not live**: Lovable's build image cannot run Chromium, and its agent committed a change that skips the prerender instead of failing. **brittmove PR #5, opened 2026-08-31, is the `react-dom/server` SSG replacement (CODE-10) — open, not merged, not deployed.** | Until PR #5 merges and a live curl confirms it (CODE-24's method), every page still serves the empty shell with one sitewide title. |
...
| **CODE-9** | **Partially resolved 2026-08-31 — the app answers now; two of the four jobs still don't finish in time.** Original finding (2026-08-29): four pg_cron jobs at `https://trumove-resource-center.lovable.app/...` all 500'd, `cron.job_run_details` reported "succeeded" regardless because it only measures whether `net.http_post` queued. **Re-measured 2026-08-31, after the operator reported every connector credential green:** `net._http_response` now shows real `200`s with real JSON for `aoos-umami-daily-observe` and `aoos-propose-from-evidence` — the endpoint is not a dead app any more, and the URL was apparently never wrong, just broken. **`aoos-gsc-daily-observe` and `aoos-ga4-daily-observe` now time out instead** (`timed_out: true`, DNS/TCP/SSL all under 60ms, then nothing before the deadline) — the shape of a live, now-credentialed round trip to a real Google API taking longer than the job was ever budgeted for, not a wrong destination. Fixed in the same session: both jobs' `timeout_milliseconds` raised to 30000 (GSC's was 15000, GA4 had none and defaulted to `net.http_post`'s 5000) — `supabase/migrations/20260831070000_raise_scheduler_hook_timeouts.sql`, applied live. **Not yet re-verified**: the next natural firing (16:05/16:35 UTC) is what proves whether 30s is enough; nobody deliberately triggered an early run to check, since that would file real proposals against production. `aoos-propose-from-evidence`'s 2026-08-30 response is itself informative and worth reading regardless: `"state":"paused","created":0,"considered":1,"message":"Required relevant active-tracked-competitor DataForSEO evidence is missing."` — correctly blocked, not broken; see CARRY-2 and OP-4. | `net._http_response` ids 58–61, 2026-08-30 data, queried 2026-08-31. `cron.job` for both jobs' updated `command`. | Verified 2026-08-31 |
...
| **CODE-24** | **Live measurement, 2026-08-31: production still serves one identical shell for every URL.** `curl` against `/`, a real service page, and a nonexistent path all return `200`, 5849 bytes, `<div id="root"></div>` empty, one sitewide `<title>`. Bigger than the 38-byte shell measured 2026-08-28 (analytics/consent scripts grew the head), but structurally the same failure -- the prerender still has never successfully shipped to this site's production deploy. CODE-10 and CODE-11 both remain fully open; nothing in source has reached a visitor yet regardless of governance state. **PR #5 (CODE-10) is the fix, opened the same day this was measured -- re-run this exact curl once it merges and deploys, before calling either item closed.** | Direct curl, 2026-08-31. | Verified |

### Website repository, continued (`maxwest525/brittmove-829a7519`)

| ID | Item | Evidence | Grade |
| --- | --- | --- | --- |
| **CODE-10** | **The prerender was merged and not running — a fix is now open, not yet live.** See OP-9. brittmove PR #5 (2026-08-31) replaces the headless-browser prerender with `react-dom/server` SSG (`renderToPipeableStream` via a new `src/entry-server.tsx`), removing the Chromium dependency Lovable's builder cannot satisfy. Verified locally with a full `bun run build`: 24 of 28 sitemap routes write real HTML with correct title/meta description/H1. Also fixed in the same PR: `/route-planning` crashed under SSR (`leaflet` touches `window` at module load); `/scanner`, `/dictate`, `/inventory-builder`, `/live-walkthrough` sit behind a client-only lead-intake gate and correctly render nothing server-side, so the script now skips them by name instead of failing the build; `/showcase` and `/plan-variants` were wrongly listed in `sitemap.xml` as public (both are staff-gated) and have been removed from it and added to `robots.txt`'s disallow list. **Not closed: unmerged, and CODE-24's live-curl method has not yet been re-run against it.** | `scripts/prerender.mjs`, `src/App.tsx`, `src/entry-server.tsx`, `src/pages/RoutePlanningPage.tsx`, brittmove PR #5 (supersedes PR #4). | Verified 2026-08-31 |

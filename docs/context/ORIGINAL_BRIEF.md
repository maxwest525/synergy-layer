# The original brief (2026-08-04)

This is the prompt the project was started from, kept verbatim because it is the
only record of what was originally asked for and several of its rules are still
binding: registries over hardcoded integrations, nothing deploys without
approval, no fake dashboards, no placeholder analytics.

Read it as history, not as a current specification. What was actually built, and
where it diverged, is in [`CURRENT_BUILD.md`](CURRENT_BUILD.md). Where the two
disagree, the code and the current-build record win.

Notable divergences already known:

- The eight numbered workspaces became a seven-slot category navigation
  (`src/lib/categories.ts`), written in plain operator words rather than in
  registry nouns. The registries survive underneath as data, not as the
  navigation.
- The product is named **Marky** on screen. **AOOS** remains the internal name
  used throughout the documentation and the schema.
- "Do not integrate dozens of APIs yet" did not hold: Search Console, GA4,
  DataForSEO, Firecrawl, Umami and SerpAPI are all wired. What did hold is the
  rule underneath it, that each one registers itself and declares its real state.

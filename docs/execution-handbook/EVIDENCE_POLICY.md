...
The current executable slice requires one allowlisted rendered page with one H1, page-level GSC evidence meeting its deterministic gate, relevant previously stored DataForSEO competitor evidence, and timestamps/checksums for all evidence. GA4 absence is disclosed but does not block proposal generation.

...
Supabase contains knowledge collections, entries, and agent-knowledge relationships, and the app can list/read them. A complete versioned chunking, embedding/retrieval, refresh, conflict, and activation pipeline is not present. State: tables `IMPLEMENTED`; governed ingestion `DESIGNED`.

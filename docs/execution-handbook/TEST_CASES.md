...
8. Approval atomically locks the selected version payload and checksum.

## Execution

9. A non-operator, wrong-tenant actor, missing approved snapshot, checksum mismatch, or non-executable state is refused.
10. Live title/H1 or content-checksum drift produces `Page changed — review required` and zero provider writes.
...
Current local tests cover observation persistence, Search Console/SEO producers, title/H1 sufficiency/evidence/generation/validation/lifecycle, Action Center view shape, execution/drift, and outcome comparisons. Database RLS/RPC behavior, live provider calls, UI persistence, and end-to-end publish proof require integration verification after authorized configuration.

  ...
  },
  {
    ...
    operations: [
      ...
      op({
        ...
        status: "wired",
        evidence: "src/lib/page-audit.server.ts",
        gap: "Still limited to the TruMove origin. The page audit now renders here first and falls back to Firecrawl only when this box is unconfigured or fails, and that fallback prefers the self-hosted Firecrawl, reaching the metered cloud only when the self-hosted base URL or key is absent. It records which renderer read each page.",
      }),
    ],
  },
  {
  ...
  },
  {
    ...
    operations: [
      ...
      op({
        ...
        status: "partial",
        mutates: true,
        evidence: "src/lib/openai-ads/capi.server.ts",
        gap: "Conversions are sent and each attempt is stored in openai_ads_deliveries, but the provider's validate-only contract is unconfirmed, so validation mode is trusted rather than proven. Every call this credential can make is a write, which is why the connector row carries no health probe.",
      }),
    ],
  },
];

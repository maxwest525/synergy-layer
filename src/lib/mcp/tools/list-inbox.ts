...
export default defineTool({
  ...
  handler: async (args, ctx) => {
    const { lane, include_resolved, limit } = args;
    return guardedRead(ctx, "list_inbox", args as Record<string, unknown>, async (supabase) => {
      let query = supabase
        .from("inbox_items")
        .select(
          "id, title, summary, lane, priority, source_module, subject_kind, subject_id, due_at, resolved_at, created_at",
        )

...
export default defineTool({
  ...
  inputSchema: {
    workflow_key: z
      .string()
      .optional()
      .describe("Restrict results to one workflow key, for example gsc-daily-observe."),
    ...
    return guardedRead(
      ...
      async (supabase) => {
        let query = supabase
          .from("workflow_runs")
          .select(
            "id, workflow_id, state, trigger_source, started_at, finished_at, duration_ms, error, workflows(key, name)",
          )

...
describe("SerpAPI workflow provider gate", () => {
  it("allows only the free provider check while pending", () => {
    ...
    expect(mayExecuteCapability("ads.advertiser_resolution", "pending")).toBe(false);
    expect(mayExecuteCapability("ads.creative_intelligence", "pending")).toBe(false);
    expect(mayExecuteCapability("ads.live_serp_observation", "pending")).toBe(false);
  });
  ...
  it("does not interfere with capabilities already proven real", () => {
    ...
    expect(mayExecuteCapability("ads.landing_page_intelligence", "real")).toBe(true);
  });

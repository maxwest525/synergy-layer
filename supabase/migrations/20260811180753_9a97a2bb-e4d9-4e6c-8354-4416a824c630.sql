-- 1. Direct writes can no longer bypass the lifecycle.
DROP POLICY IF EXISTS "Operators create change requests" ON public.change_requests;
DROP POLICY IF EXISTS "Operators update change requests" ON public.change_requests;
DROP POLICY IF EXISTS "Operators delete change requests" ON public.change_requests;

REVOKE INSERT, UPDATE, DELETE ON public.change_requests FROM authenticated;
REVOKE INSERT, UPDATE, DELETE, SELECT ON public.change_requests FROM anon;
...
-- 2. The audited transition must carry the old state as well as the new one.

-- Approval requests are decisions, not generic warnings. Move legacy open
-- workflow and agent requests into the lane that the UI and clear RPC already
-- protect, then make the RPC verify their underlying state as defense in depth.
UPDATE public.inbox_items AS item
SET lane = 'pending_approval'
WHERE item.resolved_at IS NULL
  AND item.lane = 'needs_attention'
  AND (
    (
      item.subject_kind = 'workflow_run'
      AND EXISTS (
        SELECT 1
        FROM public.workflow_runs AS run
        WHERE run.id = item.subject_id
          AND run.tenant_id = item.tenant_id
          AND run.state = 'awaiting_approval'
      )
    )
    OR (
      item.subject_kind = 'agent'
      AND EXISTS (
        SELECT 1
        FROM public.agents AS agent
        WHERE agent.id = item.subject_id
          AND agent.last_result ->> 'state' = 'awaiting_approval'
      )
    )
  );
...
BEGIN
  ...
  IF v_item.subject_kind = 'workflow_run' AND EXISTS (
    SELECT 1
    FROM public.workflow_runs AS run
    WHERE run.id = v_item.subject_id
      AND run.tenant_id = v_item.tenant_id
      AND run.state = 'awaiting_approval'
  ) THEN
    RAISE EXCEPTION 'Workflow runs waiting on approval cannot be cleared until a decision handler is wired.';
  END IF;

  IF v_item.subject_kind = 'agent' AND EXISTS (
    SELECT 1
    FROM public.agents AS agent
    WHERE agent.id = v_item.subject_id
      AND agent.last_result ->> 'state' = 'awaiting_approval'
  ) THEN
    RAISE EXCEPTION 'Agent requests waiting on approval cannot be cleared until a decision handler is wired.';
  END IF;

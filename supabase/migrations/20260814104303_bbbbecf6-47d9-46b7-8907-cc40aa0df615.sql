-- Service-only append routines for immutable change-measurement history.
-- Kept separate so each Lovable Cloud migration remains below its request limit.

CREATE OR REPLACE FUNCTION public.append_change_measurement_observation(
  _cycle_id uuid, _window_id uuid, _provider text, _source_role text, _status text,
  _payload jsonb, _source_refs jsonb, _provenance jsonb
...
END;
$$;
REVOKE ALL ON FUNCTION public.append_change_measurement_observation(uuid,uuid,text,text,text,jsonb,jsonb,jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.append_change_measurement_observation(uuid,uuid,text,text,text,jsonb,jsonb,jsonb) TO service_role;
...
END;
$$;
REVOKE ALL ON FUNCTION public.append_change_measurement_revision(uuid,uuid,uuid,text,text,jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.append_change_measurement_revision(uuid,uuid,uuid,text,text,jsonb) TO service_role;
-- The rolled_back state used to be a label an operator could stamp while the
-- site still served the new wording. It now requires a recorded revert commit,
-- so the state, and every activity line built from it, can only be reached
-- after the executor actually wrote the previous values back.

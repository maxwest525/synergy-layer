-- The rendered-proof routine re-checks the execution target so a tampered row
-- cannot widen scope, but its file list was extended one lane at a time and had
-- fallen behind GOVERNED_CHANGE_KINDS in src/lib/execution/allowlist.ts. The
-- executor commits any file a governed change kind declares, so a change request
-- on a file this routine does not list can reach a real commit on the client's
-- site and then be refused here as "outside the allowlisted execution target" --
-- the site is modified and nothing can record that it was. Restate the check as
-- the full governed file set, so this routine refuses exactly the targets the
-- executor refuses and nothing more.
--
-- Proving the change is still a separate question, answered by the wording
-- checks below: a lane with no rendered wording to compare cannot be marked
-- applied from a rendered proof, whether or not its file is listed here.
--

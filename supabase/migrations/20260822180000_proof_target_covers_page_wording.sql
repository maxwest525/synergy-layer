-- Adds the page.wording change kind to the rendered-proof routine's file list.
--
-- Until now the only page whose title and headline AOOS could fix was a service
-- page, because a service page's wording is a record in a data file. Every other
-- public page writes its wording inline in the component that renders it, so a
-- title finding on /privacy had nowhere to go: the audit reported the defect and
-- no lane could answer it.
--
-- The nineteen components below are transcribed from the client application's
-- router, one per public indexable route. Staff-gated pages, the portal, the
-- auth routes, and every page the client's own DefaultSeo marks noindex are
-- deliberately absent: the site has already said those should not be found, and
-- widening the executor to reach them would act against that.
--
-- This list must equal GOVERNED_CHANGE_KINDS in src/lib/execution/allowlist.ts.
-- The executor commits any file a governed change kind declares, so a file the
-- executor will write but this routine rejects means a real commit on the
-- client's site that nothing can then record.
--
-- src/lib/execution/proof-target-allowlist.test.ts fails if the two lists drift.

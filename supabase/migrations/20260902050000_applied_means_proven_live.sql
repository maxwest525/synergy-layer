-- Applied means proven live. There is no other way in.
--
-- Agent-runtime review 2026-09-02, AGT-2 (backlog CODE-7). The transition
-- routine still accepted `mark_applied` from any operator: approved to
-- applied with no rendered proof, a label rather than a fact, while
-- `apply_change_request_rendered_proof` is the path every real application
-- has taken (zero applied rows without a proof on 2026-09-02). The action is
-- gone from the routine; the wrapper server function and the state module
-- lose it in the same change. Everything else in the routine is byte-for-byte
-- 20260902010000.
-- Rollback: re-issue the 20260902010000 body.
CREATE OR REPLACE FUNCTION public.transition_change_request(
  _id uuid,
  _action text,
  _notes text DEFAULT NULL::text,
  _revision text DEFAULT NULL::text,
...
BEGIN
  ...
  -- Applied is reached only through apply_change_request_rendered_proof:
  -- a rendered page carrying the exact approved wording. No label does it.
  IF _action = 'mark_applied' THEN
    RAISE EXCEPTION 'Applied means proven live on the public page. Record the rendered proof; there is no mark-applied.';
  END IF;

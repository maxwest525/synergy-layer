...
await c.from("keyword_candidates").delete().eq("tenant_id", tenant!.id).eq("review_state","pending");
try {
...
console.log(rows.slice(0,30).map(r=>`${String(r.v).padStart(6)}  ${r.k}   [${r.s.replace("labs.","")}]`).join("\n"));
